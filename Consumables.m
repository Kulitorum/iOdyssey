...
- (void)sqlQueryDidFinishExecuting:(SqlClientQuery *)query
{
	...
	ServiceGroups = [NSMutableArray arrayWithCapacity: 2];
	...
	if (query.succeeded)
		...
		for (SqlResultSet *resultSet in query.resultSets)
			...
			while ([resultSet moveNext])
				...
				S->SCG_KEY = C->SCG_KEY;
				NSLog(@"Item:%@ S->SCG_KEY:%d (%@)", C->SC_NAME ,S->SCG_KEY, [resultSet getString: SCG_NAME]);
				// Do we have this key?
				bool HasIt = NO;
				...
				if(HasIt == NO)
					{
					S->SCG_NAME = [resultSet getString: SCG_NAME];
					[ServiceGroups addObject:S];
					((SCG*)[ServiceGroups objectAtIndex:([ServiceGroups count]-1)])->items = [NSMutableArray arrayWithCapacity: 2];
