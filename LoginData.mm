...
*/		
		...
		while ([resultSet moveNext])
			...
			try{
				...
				Login.PASSWRD = [resultSet getString:PASSWRD];
				...
				if ( ! [Login.USERNAME isKindOfClass:[NSString class]])
					Login.USERNAME=@"";
				Login.FULL_NAME = [resultSet getString:FULL_NAME];
				if ( ! [Login.FULL_NAME isKindOfClass:[NSString class]])
					Login.FULL_NAME=@"";
				Login.GROUP_NAME = [resultSet getString:GROUP_NAME];
				if ( ! [Login.GROUP_NAME isKindOfClass:[NSString class]])
					Login.GROUP_NAME=@"";
				Login.SITE_NAME = [resultSet getString:SITE_NAME];
				if ( ! [Login.SITE_NAME isKindOfClass:[NSString class]])
					Login.SITE_NAME=@"";
				Login.LEGAL_ENTIRY = [resultSet getString:LEGAL_ENTIRY];
				if ( ! [Login.LEGAL_ENTIRY isKindOfClass:[NSString class]])
					Login.LEGAL_ENTIRY=@"";
				Login.SECTION = [resultSet getString:SECTION];
				if ( ! [Login.SECTION isKindOfClass:[NSString class]])
					Login.SECTION=@"";
				Login.DESCRIPTION1 = [resultSet getString:DESCRIPTION1];
