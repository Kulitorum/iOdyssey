...
Date Date::LocalTime()
{
	int timeOffset = [[NSTimeZone systemTimeZone] secondsFromGMT];
	return Date(referenceTime+timeOffset);
}
...
NSString *Date::FormatForMyBookings()
{
	...
	if( IsSameDayAs(today) )
		return [NSString stringWithFormat:@"Today --->  %@ %0d %@", dayname, [components1 day], monthname];
