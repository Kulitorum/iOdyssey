...
void drawGradientInRect(CGRect rect, CGColorSpaceRef colorSpace, CGContextRef context, float r, float g, float b, float a)
{
    ...
    NSArray *array = [NSArray arrayWithObjects: (__bridge id)startColor, (__bridge id)endColor, nil];
	...
	CGColorRef frameColor = CGColorCreate(colorSpace, frameComponents);
...
-(void) DrawDateGrid:(CGRect) rect withy:(float)y withColorSpace:(CGColorSpaceRef)colorSpace WithContext:(CGContextRef) context
{
	...
	CGColorRef gridColor = CGColorCreate(colorSpace, theColor);
	CGColorRef DayColor = CGColorCreate(colorSpace, theDayColor);
	//	CGColorRef TopLineColor = CGColorCreate(colorSpace, theTopLineColor) ];	
	// Draw yellow square
	/*
    ...
    UIColor *BGColorBottom = [UIColor colorWithRed:11.0/255 green:42.0/255 blue:85.0/255 alpha:0.1];
	...
	// behind year
    UIColor *bgColorYear = [UIColor colorWithRed:129.0/255 green:146.0/255 blue:160.0/255 alpha:0.1];
	...
	// "Now" Line that shows what time it is now
...
/*
	...
	double DayNrStart = (double)(now.nstimeInterval()-dStart.nstimeInterval());	// 2.42 = 0.42 into day 2
	double tStart = DayNrStart/displayWidth.nstimeInterval();
	
	if(tStart > 1 || tStart<0)	return;// booking outside display scope
	...
	if(xs > 0)
		...
		CGFloat dashArray[] = {10,10};
		CGContextSetLineDash(context, 0, dashArray, 2);
		CGContextMoveToPoint(context, xs, HOURLINEYSTART);
		CGContextAddLineToPoint(context, xs, graphHeight);
		CGContextStrokePath(context);
		CGContextSetLineDash(context, 0, NULL, 0);	// remove dash
		CGContextSetLineWidth(context,1);
		}
...
void DrawBookingResource(CGRect rect, Resource *res, float y, float h, CGContextRef context, CGColorSpaceRef colorspace, float yStart, int resourceNr)
{
	...
	if(res.RE_KEY == AppDelegate->loginData.Login.RE_KEY)	// the patron that's logged in
		...
		CGColorRef overlayColor = CGColorCreate(colorspace, ovrColor);	
		CGContextSetFillColorWithColor(context,  overlayColor);
		CGContextFillRect (context, rectangle);
		BlueText=NO;
		}
	else if(resourceNr & 0x01)
		{
		CGRect rectangle = CGRectMake(RESOURCENAMEWIDTH,yStart,AppDelegate->ganttDisplayWidth,y-yStart);
		CGFloat ovrColor[4] = { 0,0.3f,0.0, 0.1 };
		CGColorRef overlayColor = CGColorCreate(colorspace, ovrColor);
		CGContextSetFillColorWithColor(context,  overlayColor);
		CGContextFillRect (context, rectangle);
		BlueText=NO;
	...
	/*	
	 ...
	 CGColorRef selectedColor = CGColorCreate(colorspace, selectedColorArray) ];	
...
//				AppDelegate->ganttviewcontroller.isCreatingNewBooking=YES;	// Trick colors
				...
				// Draw start time BG
					...
					UIColor *glossColor1 = [UIColor colorWithRed:1.0 green:1.0 blue:1.0 alpha:0.1];
					UIColor *glossColor2 = [UIColor colorWithRed:1.0 green:1.0 blue:1.0 alpha:0.46];
					UIColor *glossColor3 = [UIColor colorWithRed:1.0 green:1.0 blue:1.0 alpha:0.2];
					CGRect topHalf = CGRectMake(rect.origin.x, rect.origin.y, rect.size.width, rect.size.height/2);
					drawLinearGradient(context, topHalf, glossColor1, glossColor2);
...
//		[[UIColor colorWithRed:1 green:1 blue:1 alpha:1.0] setFill];
	...
	CGColorRef TextColor = CGColorCreate(colorspace, theTextColor);	
	...
	if(AppDelegate.ganttDisplaySmallLarge && AppDelegate->ganttviewcontroller.isCreatingNewBooking == NO)
		...
		if(AppDelegate.GanttSlotNamesStyle == BOOKINGNAMESTYLE)
			...
			[book.PR_NAME drawAtPoint:CGPointMake(xs+3, y) withFont:font];
...
//		CGRect scrollViewFrame = CGRectMake(RESOURCENAMEWIDTH,HOURLINEYSTART,1024-RESOURCENAMEWIDTH, 1);	// USE FOR NON-PAGE Y AXIS (check ganttScrollView.mm)
	...
	if(selectedResourceImage == nil)
		selectedResourceImage = [UIImage imageNamed:@"selectednoframe.png"];
	...
	CGColorRef patternColor = CGColorCreate(colorspace, paColor);	
	...
	for(Resource* res in AppDelegate->viewData.Resources)
		...
		if(([res.bookings count] != 0 || AppDelegate.SettingsDisplayUnbookedResources ) || res.RE_KEY == -1 )
			...
			if(y > 15 && y < AppDelegate.ganttDisplayHeight) // within the current view on the Y axis
				{
				DrawBookingResource(rect, res, y+h+4, h, context, colorspace, thisResourceStartY-4, resourceNr);
				}
			}
		if(hasDrawnResource)
			{
...
- (IBAction)RequestResourceData
{
	...
	NSString *request = [NSString stringWithFormat:@"SELECT * FROM  vw_my_resources WHERE (RV_KEY = %d OR RE_KEY = %d) AND SITE_KEY = %d AND IS_CURRENT=1 ORDER BY 3;",
