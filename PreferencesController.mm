    ...
    switch ([indexPath section]) {
		...
		switch ([indexPath row]) {
			case 0: {
				...
				if(AppDelegate.ShowLineForEveryLine == 1) dropDown4 = @"Every Resource";
                else dropDown4 =  [NSString stringWithFormat:@"%d Resources" , AppDelegate.ShowLineForEveryLine];
			...
			default: {
                ...
                NSString* label;
				if([indexPath row] == 1) label = @"Every Resource";
                else label = [NSString stringWithFormat:@"%d Resources", [indexPath row]];
