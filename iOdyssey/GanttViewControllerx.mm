...
-(void) RequestBookingData
{
	...
	NSString *request = [NSString stringWithFormat:@"SELECT * FROM  vw_staff_schedule WHERE (RV_KEY = %d OR RE_KEY = %d) AND FROM_TIME BETWEEN '%@' AND '%@' AND SITE_KEY = %d", AppDelegate.SelectedCombination, AppDelegate->loginData.Login.RE_KEY, AppDelegate.dataScopeStart.FormatForSQL(), AppDelegate.dataScopeEnd.FormatForSQL(), AppDelegate->loginData.Login.SITE_KEY];
	...
	if (query.succeeded)
		...
		for (SqlResultSet *resultSet in query.resultSets)
			...
			while ([resultSet moveNext])
				...
				/*
				 NSDate *from = [resultSet getDate:FROM_TIME];
				 NSDateFormatter* formatter = [[[NSDateFormatter alloc] init] autorelease];
				 [formatter setDateFormat:@"yyyy-MM-dd HH:mm:ss ZZZZ"];
				 NSString* str = [formatter stringFromDate:from];
				 DLog(str);
				 cout << book.FROM_TIME.str() << endl;
				 */
...
-(void) willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
	...
	switch (toInterfaceOrientation)
	{
		...
		cout << "Will: presentModalViewController:MyBookingsAndDrillDown" << endl;
		if([AppDelegate isDoneGettingInitData] == NO)
			[self presentModalViewController:AppDelegate.MyBookingsAndDrillDown animated:NO];
		else 
...
/*
	...
	asd.frame = CGRectMake(0,248,1024,476);
	[asd setNeedsLayout];
